...
你 clone 了很多项目。每个项目里散落着各种文档——`.md` 文件、`.pdf` 参考资料——藏在层层嵌套的目录里。你想在一个地方阅读和检索它们，但它们分布在几十个项目文件夹中。

**obsidian-sync** 一条命令，把项目中所有文档文件同步到你的 Obsidian vault，同时保持项目中的正常访问。
...
| `--extensions 扩展名` | 同步的文件类型（逗号分隔） | `.md,.pdf` |
| `--dry-run` | 仅预览，不执行 | 关闭 |

## 同步范围

**同步**：默认同步所有 `.md` 和 `.pdf` 文件。可通过 `--extensions` 自定义。
